...
def plot_width_histograms(data_dict, percent_data_dict, output_dir: str = "./results"):
    ...
    # Check if we have the right number of categories
    if len(data_dict) != 6:
        raise ValueError(f"Expected 6 categories, but got {len(data_dict)}.")
    
    # Create figure and grid (3x2 categories, each with 2 plots)
    fig = plt.figure(figsize=(15, 18))  # Taller figure to accommodate more plots
    gs = GridSpec(6, 2, figure=fig)  # 6 rows, 2 columns
    
    # Get all category IDs sorted
    categories = sorted(data_dict.keys())
    
    # Plot position mapping
    for i, category in enumerate(categories):
        col = i % 2  # 0 or 1
        row = i // 2 * 2  # 0, 2, or 4 (doubled to make room for two plots per category)
...
# Update the main function to include the new percent differences
def main():
